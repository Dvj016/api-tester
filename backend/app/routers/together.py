...
async def get_models():
    """Get list of popular Together AI models"""
    return {"models": TOGETHER_MODELS}


@router.post("/test", response_model=TestResponse)
async def test_together_key(request: TestRequest):
    """
    Test a Together AI API key
    ...
    logger.info(f"Testing Together AI API key: {masked_key} with model: {request.model}")
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            logger.info(f"Together AI key {masked_key} is valid. Latency: {latency_ms:.2f}ms")
            ...
            logger.warning(f"Together AI key {masked_key} authentication failed")
            return TestResponse(
                valid=False,
                provider="together",
                model=request.model,
        ...
        logger.error(f"Timeout testing Together AI key {masked_key}")
        return TestResponse(
            valid=False,
            provider="together",
            model=request.model,
